...
// For this exercise use the `filter` , `map`, y `reduce` methods 
// to generate some reports.

// Create a `cars.js` file with the following content:


// 1. Create a function `ferraris` that returns all the cars of the
//  Ferrari brand.


// 2. Create a function `nineties` that returns all the cars with models
//  between 1980 and 1990.


// 3. Create a function `list` that returns an array of strings with the 
// same number of cars. Each element should contain the string 
// “This <brand> from <model> costs $<price>” (e.g. for the first 
//     element it should say “This Mazda from 1989 costs $5241”).
    
// 3. Create a function `bmwSum` that returns the sum of all prices of 
// the BMW cars.

const ferraris = (cars) => {
    const result = cars.filter(elem => elem.brand === 'Ferrari');
    return result;
};

const nineties = (cars) => {
    let result = cars.filter(elem => elem.model >= 1980);
    result = result.filter(elem => elem.model <= 1990);
    return result;
};

const list = (cars) => {
    const result = cars.map(elem => "This " + elem.brand + " from " + elem.model + " costs " + elem.price);
    return result;
};
