...
class Company{
    ...
    generatePayroll(){
        const paymentsE = this.employees.map(elem => elem.baseSalary * 0.88);
        const paymentE = paymentsE.reduce((acc,cur) => acc + cur);
        ...
        const paymentsM = this.managers.map(elem => elem.baseSalary);
        ...
        const result = paymentE + paymentM;
